...
class _ImageUploadState extends State<ImageUploadWidget> {
  ...
  void pickImage() async {
    ...
    final imageName = 'back_${DateUtil.getYMD(DateTime.now())}';
    ...
    try {
      ...
      while (!analysisResult) {
        ...
        if (analysisResult) {
          setState(() {
            analyzed2 = true;
            widget.onAnalysisChanged(analyzed2);
          });
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
      ...
      child: uploading
          ...
          : (imageUpload
          ? ImageProcessingWidget(uploadImageUrl, !analyzed2)
          : const Text("CLICK TO UPLOAD", style: TextStyle(fontSize: 16, color: Colors.black54))),
    );
  }
}

class Scalp2Screen extends StatefulWidget {
  const Scalp2Screen({super.key});

  @override
  State<Scalp2Screen> createState() => _Scalp2ScreenState();
}

class _Scalp2ScreenState extends State<Scalp2Screen> {
  bool isAnalyzed2 = false;

  void handleAnalysisChanged(bool analyzed2) {
    setState(() {
      isAnalyzed2 = analyzed2;
    });
  }
                  ...
                  child: ElevatedButton(
                    ...
                    style: ButtonStyle(
                      ...
                      shape: MaterialStateProperty.all<RoundedRectangleBorder>(
                        RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(18.0),
                          side: BorderSide(color: isAnalyzed2 ? Colors.green : Colors.grey), // Transparent border when disabled
                        ),
                      ),
