...
class _ImageUploadState extends State<ImageUploadWidget> {
  ...
  void pickImage() async {
    ...
    final imageName = 'right_${DateUtil.getYMD(DateTime.now())}';
    ...
    try {
      ...
      while (!analysisResult) {
        ...
        if (analysisResult) {
          setState(() {
            analyzed4 = true;
            widget.onAnalysisChanged(analyzed4);
          });
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
      ...
      child: uploading
          ...
          : (imageUpload
          ? ImageProcessingWidget(uploadImageUrl, !analyzed4)
          : const Text("CLICK TO UPLOAD", style: TextStyle(fontSize: 16, color: Colors.black54))),
    );
  }
}

class Scalp4Screen extends StatefulWidget {
  const Scalp4Screen({super.key});

  @override
  State<Scalp4Screen> createState() => _Scalp4ScreenState();
}

class _Scalp4ScreenState extends State<Scalp4Screen> {
  bool isAnalyzed4 = false;

  void handleAnalysisChanged(bool analyzed4) {
    setState(() {
      isAnalyzed4 = analyzed4;
    });
  }
            ...
            child: ListView(children: [
              ...
              Center(
                child: ElevatedButton(
                  onPressed: isAnalyzed4 ? () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) =>  ScalpResultScreen(sculpKey: DateUtil.getYMD(DateTime.now())) )
                    );
                  ...
                  style: ButtonStyle(
                    ...
                    shape: MaterialStateProperty.all<RoundedRectangleBorder>(
                      RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(18.0),
                        side: BorderSide(color: isAnalyzed4 ? Colors.green : Colors.grey), // Transparent border when disabled
                      ),
                    ),
