...
class Alignment(nn.Module):
    ...
    #added - 1221 MJ Lee
    def shift_hairmask(self, mask, shift_distance):
        mask_cpu = mask.cpu()
        mask_array = mask_cpu.numpy()
        shift_sequence = (shift_distance, 0)  # 예시: 왼쪽으로 shift_distance만큼 이동
        shifted_mask_array = shift(mask_array, shift_sequence, mode='constant', cval=0)
        shifted_mask_tensor = torch.tensor(shifted_mask_array)

        return shifted_mask_tensor
    ...
    def align_images(self, img_path1, img_path2, sign='realistic', align_more_region=False, smooth=5,
        ...
        if self.opts.blend_with_align:
            ...
            for step in pbar:
                ...
                # Shifting source hairline segment -> Failed!
                # shift_distance = 100  # 헤어마스크를 아래로 이동시킬 거리 설정
                # new_target_hairmask_down_256 = self.shift_hairmask(target_hairmask_down_256, shift_distance)
    ...
    def optimize_src_latent_with_aligned_mask(self, latent_W_path_1, target_mask, latent_1, is_downsampled):
        ...
        pbar = tqdm(range(20), desc='Align Step 1', leave=False) 
